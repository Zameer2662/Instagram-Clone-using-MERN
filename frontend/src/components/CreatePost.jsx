...
const CreatePost = ({ open, setOpen }) => {
    ...
    const createPostHandler = async (e) => {
        ...
        if (!caption.trim()) {
            return toast.error("Caption cannot be empty.");
        }
        if (!file) {
        ...
        try {
            ...
            if (res.data.success) {
                dispatch(setPosts([ res.data.post, ...posts])) // array mainn new photo add krta jaye ga 
    ...
    return (
        <Dialog open={open}>
            <DialogContent onInteractOutside={() => setOpen(false)}>
                ...
                <Button onClick={() => imageRef.current.click()} className="w-fit mx-auto bg-[#0095f6] hover:bg-[#0094f6cd]">
